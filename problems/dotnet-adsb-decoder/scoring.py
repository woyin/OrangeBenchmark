"""Custom scoring for .NET ADS-B Decoder problem."""
...
def score_performance(generated_code: str, work_dir: str) -> float:
    """Decode 10k messages and measure time. Score 1.0 if <1s, degrade to 0 at 3s."""
    if shutil.which("dotnet") is None:
        return 0.0
    try:
        build = _run(["dotnet", "build", "--verbosity", "quiet"], work_dir)
        ...
        # Run the test project to check performance via the test method
        test_script = """
using System;
...
"""
        ...
        # Modify csproj to make it a console app with PerfTest entry point
        result = _run(
        ...
        if ms < 1000:
            ...
            return round(1.0 - (ms - 1000) / 2000.0, 4)
    except Exception:
        return 0.5
